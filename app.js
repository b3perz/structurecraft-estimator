...
// ---- SECTION 8B: EVENT DELEGATION SYSTEM ----
var ACTION_HANDLERS = {};
...
function initDelegation() {
  ...
  main.addEventListener('dragstart', function(e) {
    ...
    if (!t) return;
    var action = t.getAttribute('data-dragstart');
    var raw = t.getAttribute('data-params');
    var params = raw ? JSON.parse(raw) : {};
    if (ACTION_HANDLERS[action]) ACTION_HANDLERS[action](params, t, e);
  });

  // Body-level delegation for elements outside main/sidebar (e.g., notification panels, modals)
  document.body.addEventListener('click', function(e) {
    var t = e.target.closest('[data-action]');
    if (!t) return;
    // Skip if already handled by main or sidebar delegation
    if (main.contains(t) || document.getElementById('sidebar').contains(t)) return;
    var action = t.getAttribute('data-action');
    var raw = t.getAttribute('data-params');
    var params = raw ? JSON.parse(raw) : {};
    if (ACTION_HANDLERS[action]) ACTION_HANDLERS[action](params, t, e);
...
registerAction('updateFootbridgeConfig', function(p) {
  var val = p.type === 'int' ? parseInt(p._value) : parseFloat(p._value);
...
registerAction('setOutputTab', function(p) { STATE.outputActiveTab = p.tab; renderPage(); });
...
registerAction('connectEstimate', function(p) { connectEstimate(p._value); });
...
registerAction('quickAddFromConnector', function(p) { quickAddFromConnector(p.phaseKey, p.idx); });
registerAction('setTheme', function(p) { setTheme(p.id); });
registerAction('toggleGuide', function() { toggleGuide(); });
registerAction('showNotificationsPanel', function() { showNotificationsPanel(); });
registerAction('togglePricingCategory', function(p, target) { togglePricingCategory(target); });
...
registerAction('triggerImportFile', function() { document.getElementById('import-file').click(); });
registerAction('confirmClearData', function() { confirmClearData(); });
registerAction('removeFile', function(p) { removeFile(p.category, p.idx); });
registerAction('addLineItem', function(p) { addLineItem(p.phaseKey); });
registerAction('removeLineItem', function(p) { removeLineItem(p.phaseKey, p.idx); });
registerAction('toggleItemRationale', function(p) { toggleItemRationale(p.phaseKey, p.idx); });
registerAction('toggleVolRationale', function(p, target) { toggleVolRationale(target.closest('tr')); });
...
registerAction('connectEstimateById', function(p, target, e) { e.stopPropagation(); connectEstimate(p.id); });
registerAction('duplicateEstimate', function(p, target, e) { e.stopPropagation(); duplicateEstimate(p.id); });
registerAction('loadEstimateToWorkspace', function(p, target, e) { e.stopPropagation(); loadEstimateToWorkspace(p.id); });
registerAction('toggleQA', function(p) { toggleQA(p.idx); });
registerAction('switchOutputTab', function(p) { switchOutputTab(p.tab); });
registerAction('executePaletteCommand', function(p) { executePaletteCommand(p.idx); });
registerAction('updateLineItem', function(p) {
  var val = p.valType === 'float' ? parseFloat(p._value) : p._value;
  updateLineItem(p.phaseKey, p.idx, p.field, val);
});
registerAction('filterPastModel', function(p) { STATE.pastFilterModel = p._value; renderPage(); });
registerAction('filterPastStatus', function(p) { STATE.pastFilterStatus = p._value; renderPage(); });
registerAction('saveAnthropicModel', function(p) { localStorage.setItem('sc-anthropic-model', p._value); });
registerAction('saveAPIKey', function(p) { saveAPIKey(p._value); });
registerAction('setFbState', function(p) { STATE[p.key] = p._value; });
registerAction('viewFootbridgeOutput', function() {
...
registerAction('viewQueueFromNotif', function() { navigateTo('queue'); var el = document.getElementById('notif-panel'); if (el) el.remove(); });
...
registerAction('importData', function(p, target) { importData({ target: target }); });
...
registerAction('handleFileSelect', function(p, target, e) { handleFileSelect(e, p.category); });
registerAction('dragConnectorItem', function(p, target, e) { e.dataTransfer.setData('text/plain', JSON.stringify({ phaseKey: p.phaseKey, idx: p.idx })); });
registerAction('scottDismiss', function() { scottDismiss(); });
registerAction('scottToggle', function() { scottToggle(); });
registerAction('handleDrop', function(p, target, e) { handleDrop(e, p.category); });
registerAction('dropConnectorItem', function(p, target, e) { dropConnectorItem(e); });
...
function renderInputPage() {
  ...
  return '<div class="fade-in">' +
        ...
        '<div class="form-group">' +
          ...
          '<div class="material-toggle-group">' +
            Object.entries(PRIMARY_MATERIALS).map(function(entry) {
              ...
              return '<button class="material-toggle-btn ' + ((est.primaryMaterial || 'timber') === key ? 'active' : '') + '" data-action="setPrimaryMaterial" data-params=\'{"category":"' + key + '"}\' title="' + mat.description + '">' + mat.name + '</button>';
            }).join('') +
          '</div>' +
        '</div>' +
      '</div>' +
    ...
    '<div class="upload-grid mb-16">' +
      '<div class="card">' +
        '<div class="upload-zone" id="upload-rfp" data-drop="handleDrop" data-dragover="1" data-params=\'{"category":"rfp"}\' data-action="triggerUpload">' +
      ...
      '</div>' +
      '<div class="card">' +
        '<div class="upload-zone" id="upload-drawings" data-drop="handleDrop" data-dragover="1" data-params=\'{"category":"drawings"}\' data-action="triggerUpload">' +
          '<div class="upload-icon">' + ICONS.upload + '</div>' +
      ...
      '</div>' +
      '<div class="card">' +
        '<div class="upload-zone" id="upload-structural" data-drop="handleDrop" data-dragover="1" data-params=\'{"category":"structural"}\' data-action="triggerUpload">' +
          '<div class="upload-icon">' + ICONS.upload + '</div>' +
      ...
      '</div>' +
      '<div class="card">' +
        '<div class="upload-zone" id="upload-narratives" data-drop="handleDrop" data-dragover="1" data-params=\'{"category":"narratives"}\' data-action="triggerUpload">' +
          '<div class="upload-icon">' + ICONS.upload + '</div>' +
    ...
    benchmarkCard = '<div class="card mb-16" style="border-left: 4px solid ' + statusColor + ';">' +
      ...
      validation.flags.map(function(flag) {
        ...
        return '<div style="display:flex; align-items:flex-start; gap:6px; font-size:0.75rem; line-height:1.5; padding:3px 0;">' +
          ...
          '<span style="color:var(--text-secondary);">' + esc(flag.message) + '</span>' +
        '</div>';
      }).join('') +
      '</div>' +
    '</div>';
      ...
      phase.items.map(function(item, idx) {
        ...
        return '<div class="line-item-wrapper" id="rationale-' + phaseKey + '-' + idx + '">' +
          '<div class="line-item-row" draggable="true">' +
            ...
            '<div><input class="line-item-input numeric" type="number" value="' + item.qty + '" data-change="updateLineItem" data-params=\'{"phaseKey":"' + phaseKey + '","idx":' + idx + ',"field":"qty","valType":"float"}\'></div>' +
            ...
            '<div><input class="line-item-input numeric" type="number" step="0.01" value="' + item.rate + '" data-change="updateLineItem" data-params=\'{"phaseKey":"' + phaseKey + '","idx":' + idx + ',"field":"rate","valType":"float"}\'></div>' +
      ...
      '<div style="padding: 4px 0;">' +
        notes.map(function(note, i) {
          ...
          return '<div style="display: flex; align-items: flex-start; gap: 10px; padding: 10px 12px; margin-bottom: 4px; background: ' + bg + '; border-radius: 6px; border-left: 3px solid ' + color + ';">' +
            ...
            '<span style="font-size: 0.82rem; color: var(--text-primary); line-height: 1.5;">' + esc(note) + '</span>' +
          '</div>';
        }).join('') +
      '</div>' +
    '</div>' +
  ...
  return '<div class="fade-in">' +
    ...
    '<div class="card mb-16" style="padding: 12px 16px;">' +
      '<div class="flex items-center gap-12 flex-wrap">' +
        ...
        '<select class="form-select" style="width: auto; min-width: 160px; padding: 5px 30px 5px 10px;" data-change="filterPastModel">' +
          '<option value="all">All Models</option>' +
          Object.entries(DELIVERY_MODELS).map(function(entry) {
            return '<option value="' + entry[0] + '"' + (filterModel===entry[0]?' selected':'') + '>' + entry[1].name + '</option>';
          }).join('') +
        '</select>' +
        '<select class="form-select" style="width: auto; min-width: 120px; padding: 5px 30px 5px 10px;" data-change="filterPastStatus">' +
...
function renderEstimateCard(est) {
  ...
  return '<div class="estimate-card" data-action="viewEstimateDetail" data-params=\'{"id":"' + est.id + '"}\'>' +
    ...
    '<div style="margin-top:10px; display:flex; gap:6px;">' +
      '<button class="btn btn-sm btn-ghost" data-action="connectEstimateById" data-params=\'{"id":"' + est.id + '"}\'>' + ICONS.link + ' Connect</button>' +
...
function renderConnectorPage() {
      ...
      Object.entries(connected.phases).forEach(function(entry) {
        ...
        if (phase.items) {
          phase.items.forEach(function(item, idx) {
            itemsHtml += '<div class="connector-item" draggable="true" data-dragstart="dragConnectorItem" data-dblclick="quickAddFromConnector" data-params=\'{"phaseKey":"' + phaseKey + '","idx":' + idx + '}\'>' +
  ...
  return '<div class="fade-in">' +
    ...
    '<div class="grid-2">' +
      ...
      '<div>' +
        '<div class="card" style="min-height: 300px;" data-drop="dropConnectorItem" data-dragover="1">' +
  ...
  return '<div class="fade-in">' +
    ...
    '<div class="card mb-20"><div class="card-header"><div><div class="card-title">' + ICONS.bolt + ' AI Configuration</div><div class="card-subtitle">Connect Claude AI for intelligent PDF analysis and quantity takeoff</div></div></div>' +
      ...
      '<div class="form-row">' +
        '<div class="form-group">' +
          ...
          '<select class="form-select" data-change="saveAnthropicModel">' +
    ...
    '<div class="card mb-20"><div class="card-header"><div class="card-title">Data Management</div></div>' +
      '<div class="flex gap-8 flex-wrap">' +
        ...
        '<button class="btn" data-action="triggerImportFile">' + ICONS.upload + ' Import Data</button>' +
...
function renderFootbridgePage() {
  ...
  if (fbEst && fbEst.phases && Object.keys(fbEst.phases).length > 0) {
    ...
    outputHtml = '<div class="section-divider mt-24">Estimate Generated</div>' +
      '<div class="card" style="border-left: 4px solid var(--accent); display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:16px;">' +
        ...
        '</div>' +
        '<button class="btn btn-accent" data-action="viewFootbridgeOutput">' +
  ...
  return '<div class="fade-in">' +
    ...
    '<div class="card mb-16">' +
      ...
      '<div class="form-row" style="margin-bottom:16px;">' +
        '<div class="form-group" style="flex:2;">' +
          ...
          '<select class="form-select" data-change="updateFootbridgeConfig" data-params=\'{"key":"bridgeType"}\'>' +
    ...
    '<div class="card mb-16">' +
      ...
      '<div class="form-row">' +
        '<div class="form-group"><label class="form-label">Project Name</label><input class="form-input" type="text" id="fb-project-name" placeholder="e.g., Stanley Park Pedestrian Bridge" value="' + esc(STATE.fbProjectName || '') + '" data-change="setFbState" data-params=\'{"key":"fbProjectName"}\'></div>' +
        '<div class="form-group"><label class="form-label">Client</label><input class="form-input" type="text" id="fb-client" placeholder="e.g., Vancouver Parks Board" value="' + esc(STATE.fbClient || '') + '" data-change="setFbState" data-params=\'{"key":"fbClient"}\'></div>' +
        '<div class="form-group"><label class="form-label">Location</label><input class="form-input" type="text" id="fb-location" placeholder="e.g., Vancouver, BC" value="' + esc(STATE.fbLocation || '') + '" data-change="setFbState" data-params=\'{"key":"fbLocation"}\'></div>' +
      '</div>' +
      '<div class="form-row">' +
        '<div class="form-group">' +
...
function renderFootbridgePhaseTab(fbEst, phaseKey) {
  ...
  return '<div class="slide-up"><div class="card">' +
    ...
    '<div class="line-items-header" style="grid-template-columns: 40px 1fr 80px 80px 100px 120px;"><div></div><div>Description</div><div class="cell-right">Qty</div><div class="cell-center">Unit</div><div class="cell-right">Rate</div><div class="cell-right">Total</div></div>' +
    phase.items.map(function(item) {
      return '<div class="line-item-row" style="grid-template-columns: 40px 1fr 80px 80px 100px 120px;">' +
        '<div></div>' +
    ...
    (phase.items.length > 0 ? '<div class="line-item-row subtotal-row" style="grid-template-columns: 40px 1fr 80px 80px 100px 120px;"><div></div><div style="font-weight:700; color:var(--text-primary);">Phase Subtotal</div><div></div><div></div><div></div><div class="cell-right cell-mono cell-accent" style="padding:4px 6px; font-size:0.92rem;">' + fmt(phaseSubtotal) + '</div></div>' : '') +
  '</div></div>';
}
...
function addLineItem(phaseKey) {
  ...
  var newIdx = STATE.currentEstimate.phases[phaseKey].items.length;
  ...
  requestAnimationFrame(function() {
    var wrapper = document.getElementById('rationale-' + phaseKey + '-' + newIdx);
    if (wrapper) {
      var firstInput = wrapper.querySelector('.line-item-input');
      if (firstInput) {
        firstInput.focus();
        firstInput.select();
      }
...
function updateLineItem(phaseKey, idx, field, value) {
  ...
  saveState();
  // In-place DOM update: only touch the cells that changed, preserve focus/cursor
  _updateLineItemDOM(phaseKey, idx, item);
}

function _updateLineItemDOM(phaseKey, idx, item) {
  // Update the total cell for this row
  var wrapper = document.getElementById('rationale-' + phaseKey + '-' + idx);
  if (!wrapper) { renderPage(); return; }
  var row = wrapper.querySelector('.line-item-row');
  if (!row) { renderPage(); return; }
  // Total is the 7th column (index 6) in the 8-column grid
  var totalCell = row.children[6];
  if (totalCell && !totalCell.querySelector('input')) {
    totalCell.textContent = fmt(item.total);
  }
  // Update phase subtotal row
  var phaseCard = wrapper.parentElement;
  if (phaseCard) {
    var subtotalRow = phaseCard.querySelector('.subtotal-row');
    if (subtotalRow) {
      var subtotalCell = subtotalRow.children[6];
      if (subtotalCell) {
        subtotalCell.textContent = fmt(calcPhaseTotal(STATE.currentEstimate.phases[phaseKey]));
      }
    }
  }
}
  ...
  var html = '<div class="notif-panel" id="notif-panel">' +
    ...
    (inProgress.length > 0 ?
      '<div style="padding:12px 16px; background:var(--accent-muted); border-bottom:1px solid var(--border);">' +
        ...
        '<button class="btn btn-sm btn-accent" style="margin-top:8px;" data-action="viewQueueFromNotif">View Queue</button>' +
      '</div>'
    : '') +
...
// ---- FOCUS PRESERVATION ----
// Saves enough info about the currently focused element to relocate it after a full re-render.
function _saveFocusState() {
  var el = document.activeElement;
  if (!el || el === document.body || el.tagName === 'BODY') return null;
  var info = { tag: el.tagName, id: el.id, selStart: null, selEnd: null };
  // For inputs inside line-item rows, record the row index and column position
  var wrapper = el.closest && el.closest('.line-item-wrapper');
  if (wrapper && wrapper.id) {
    info.wrapperId = wrapper.id;
    // Find which input within the row
    var row = el.closest('.line-item-row');
    if (row) {
      var inputs = row.querySelectorAll('input, select');
      for (var i = 0; i < inputs.length; i++) {
        if (inputs[i] === el) { info.inputIndex = i; break; }
      }
    }
  }
  // For rationale inputs
  var rationale = el.closest && el.closest('.line-item-rationale');
  if (rationale) {
    var rWrapper = rationale.closest('.line-item-wrapper');
    if (rWrapper && rWrapper.id) {
      info.wrapperId = rWrapper.id;
      info.inRationale = true;
      var rInputs = rationale.querySelectorAll('input, select');
      for (var ri = 0; ri < rInputs.length; ri++) {
        if (rInputs[ri] === el) { info.rationaleInputIndex = ri; break; }
      }
    }
  }
  // Save cursor position within text inputs
  if (el.tagName === 'INPUT' || el.tagName === 'TEXTAREA') {
    try {
      info.selStart = el.selectionStart;
      info.selEnd = el.selectionEnd;
    } catch(e) { /* number inputs don't support selectionStart */ }
  }
  return info;
}
...
function _restoreFocusState(info) {
  ...
  requestAnimationFrame(function() {
    var target = null;
    // Try to find the element by wrapper ID + input index (line items)
    if (info.wrapperId) {
      var wrapper = document.getElementById(info.wrapperId);
      if (wrapper) {
        if (info.inRationale) {
          var rationale = wrapper.querySelector('.line-item-rationale');
          if (rationale) {
            var rInputs = rationale.querySelectorAll('input, select');
            if (info.rationaleInputIndex != null && rInputs[info.rationaleInputIndex]) {
              target = rInputs[info.rationaleInputIndex];
            }
          }
        } else {
          var row = wrapper.querySelector('.line-item-row');
          if (row) {
            var inputs = row.querySelectorAll('input, select');
            if (info.inputIndex != null && inputs[info.inputIndex]) {
              target = inputs[info.inputIndex];
            }
          }
        }
      }
    ...
    if (target) {
      ...
      if (info.selStart != null && (target.tagName === 'INPUT' || target.tagName === 'TEXTAREA')) {
        try {
          target.setSelectionRange(info.selStart, info.selEnd);
        } catch(e) { /* number inputs don't support setSelectionRange */ }
      }
    }
  });
}
...
function initKeyboardShortcuts() {
  document.addEventListener('keydown', function(e) {
    ...
    // Line-item grid keyboard navigation: Tab, Shift+Tab, Enter, Arrow keys
    if (isInput && e.target.closest && e.target.closest('.line-item-row, .line-item-rationale')) {
      var navHandled = _handleLineItemNav(e);
      if (navHandled) return;
    }

    // Escape while editing a line-item input: blur without re-render
...
// ---- SECTION 14B: LINE ITEM GRID NAVIGATION ----
// Handles Tab, Shift+Tab, Enter, and vertical Arrow keys within line-item inputs.
// Returns true if the event was handled (caller should stop propagation).
function _handleLineItemNav(e) {
  var el = e.target;
  if (e.key !== 'Tab' && e.key !== 'Enter' && e.key !== 'ArrowDown' && e.key !== 'ArrowUp') return false;

  // Collect all editable inputs across the entire phase card
  var card = el.closest('.card');
  if (!card) return false;
  var allInputs = Array.from(card.querySelectorAll('.line-item-row input, .line-item-row select'));
  if (allInputs.length === 0) return false;

  var curIdx = allInputs.indexOf(el);
  if (curIdx === -1) return false;

  // Count columns per row (inputs in a single .line-item-row)
  var row = el.closest('.line-item-row');
  var rowInputs = row ? Array.from(row.querySelectorAll('input, select')) : [];
  var colCount = rowInputs.length;
  var colIdx = rowInputs.indexOf(el);
  ...
  if (e.key === 'Tab') {
    e.preventDefault();
    if (e.shiftKey) {
      nextTarget = allInputs[curIdx - 1] || allInputs[allInputs.length - 1];
    } else {
      nextTarget = allInputs[curIdx + 1] || allInputs[0];
    }
  } else if (e.key === 'Enter') {
    e.preventDefault();
    // Enter moves down to the same column in the next row
    var nextIdx = curIdx + colCount;
    nextTarget = allInputs[nextIdx] || null;
    if (!nextTarget) {
      // At the last row — just blur
      el.blur();
      return true;
    }
  } else if (e.key === 'ArrowDown') {
    var downIdx = curIdx + colCount;
    if (allInputs[downIdx]) {
      e.preventDefault();
      nextTarget = allInputs[downIdx];
    } else {
      return false;
    }
  } else if (e.key === 'ArrowUp') {
    var upIdx = curIdx - colCount;
    if (allInputs[upIdx]) {
      e.preventDefault();
      nextTarget = allInputs[upIdx];
    } else {
      return false;
    }
  }
  ...
  if (nextTarget) {
    // Trigger onchange on current element before moving (fire any pending update)
    if (el !== nextTarget) {
      el.blur();
    }
    nextTarget.focus();
